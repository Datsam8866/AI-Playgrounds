...
Train window: prior 5 seasons
Calibration: Isotonic regression on Y-1 raw probabilities
Test years: 2016..2025 by default
...
CSV_PATH = Path(__file__).resolve().parent / "nba_walkforward.csv"
REPORT_PATH = Path(__file__).resolve().parent / "nba_model_report.md"
...
def parse_args():
    parser = argparse.ArgumentParser(description="NBA walk-forward XGBoost backtest.")
    parser.add_argument(
        "--year",
        type=int,
        help="Run only one test year (e.g. 2025).",
    )
...
def walkforward(all_rows, target_year=None):
    ...
    print("=== NBA Walk-Forward XGBoost ===")
    for test_year in years:
        ...
        train_label = f"{fold['train_years'][0]}-{fold['train_years'][-1]}"
    ...
    overall_cal = sum(row["correct"] for row in all_results) / len(all_results) if all_results else 0.0
...
def write_report(year_stats, results, importance_accumulator):
    overall_raw = sum(row["correct_raw"] for row in results) / len(results) if results else 0.0
    overall_cal = sum(row["correct"] for row in results) / len(results) if results else 0.0
    ...
    overall_brier = brier_score(calibrated_probs, actuals) if results else 0.0
    overall_ece = expected_calibration_error(calibrated_probs, actuals, n_bins=10) if results else 0.0
    ...
    for stat in year_stats:
        train_label = f"{stat['train_years'][0]}-{stat['train_years'][-1]}"
        lines.append(
