# CLAUDE.md — Baseball 專案爬蟲規則

## 爬蟲工具選擇原則

| 情境 | 工具 | 使用方式 |
|---|---|---|
| 社群媒體（PTT、Twitter/X、Facebook 等） | **Playwright MCP** | 直接叫 Claude 操作，或寫 Python 腳本 |
| 一般靜態/半靜態網站 | **Firecrawl MCP** | 直接叫 Claude 爬取，或寫 Python 腳本 |
| CPBL 官網 API（`/box/getlive` 等） | **requests** | 已知 API 端點，不需要瀏覽器 |

## MCP 設定（全域 user 層級）

Playwright 與 Firecrawl 已安裝為 **全域 MCP server**（`~/.claude.json`），在任何專案皆可直接使用。
...
**使用方式**：直接對 Claude 下指令，例如：
- 「用 Firecrawl 爬這個網址，給我 Markdown」
- 「用 Playwright 打開這個頁面，截圖給我」

重新啟動 Claude Code 後 MCP 即生效，無需額外設定。
...
claude mcp add firecrawl -s user -e FIRECRAWL_API_KEY=fc-0e79b5dee91e44979373f6052b9cd351 -- npx firecrawl-mcp
```
